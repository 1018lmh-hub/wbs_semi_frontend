...
export function makeMarkerClustering(naver) {
  ...
  naver.maps.Util.ClassExtend(MarkerClustering, naver.maps.OverlayView, {
    ...
    setIndexGenerator: function (ig) {
      this.setOptions("indexGenerator", ig);
    },
    getMarkers: function () {
    ...
    setAverageCenter: function (ac) {
      this.setOptions("averageCenter", ac);
    },
  ...
  Cluster.prototype = {
    ...
    updateCluster: function () {
      if (!this._clusterMarker) {
        var position = this._markerClusterer.getAverageCenter()
    ...
    _calcAverageCenter: function (markers) {
      var n = markers.length,
        avg = [0, 0];
      for (var i = 0; i < n; i++) {
        avg[0] += markers[i].position.x;
        avg[1] += markers[i].position.y;
      }
      avg[0] /= n;
      avg[1] /= n;
